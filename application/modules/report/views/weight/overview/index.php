...
<tr>
	...
	<td><?php $td8 = $total[$item['area_no']][1]; echo number_format($td8); $t8[] = $td8 ?></td>
	<td><?php $td9 = (empty($user_total[$item['area_no']][1])) ? 0 : $user_total[$item['area_no']][1]; echo number_format($td9);$t9[] =$td9; ?></td>
	<td><?php echo (empty($td8) || empty($td9)) ? 0 : number_format(($td9*100)/$td8,1) ?></td>
	<td><?php $td11 = (empty($fat[$item['area_no']]['อ้วนลงพุง'][1])) ? 0 : $fat[$item['area_no']]['อ้วนลงพุง'][1]; echo number_format($td11); $t11[]=$td11; ?></td>
	<td><?php echo $td12[$key] =(empty($td11) || empty($td9)) ? 0 : number_format(($td11*100)/$td9,1) ?></td>
	<td><?php $td13[$key]  =(empty($waistline[$item['area_no']][1])) ? 0 : $waistline[$item['area_no']][1]; echo number_format($td13[$key],1); $t13[] = $td13[$key]; ?></td>
	<td><?php $td14[$key] = (empty($weight[$item['area_no']]['sum'][1])) ? 0 : $weight[$item['area_no']]['sum'][1]; echo number_format($td14[$key]); $t14[] = $td14[$key]; ?></td>
	<td><?php $td15[$key] = (empty($weight[$item['area_no']]['avg'][1])) ? 0 : $weight[$item['area_no']]['avg'][1]; echo number_format($td15[$key],1); $t15[] = $td15[$key]; ?></td>
	<td><?php $td16 = (empty($sd[$item['area_no']][1])) ? 0 : $sd[$item['area_no']][1]; echo number_format($td16,1);$t16[] = $td16; ?></td>


	<td><?php $td17 = $total[$item['area_no']][2]; echo number_format($td17); $t17[] = $td17 ?></td>
	<td><?php $td18 = (empty($user_total[$item['area_no']][2])) ? 0 : $user_total[$item['area_no']][2]; echo number_format($td18);$t18[] =$td18; ?></td>
	<td><?php echo (empty($td17) || empty($td18)) ? 0 : number_format(($td18*100)/$td17,1) ?></td>
	<td><?php $td20 = (empty($fat[$item['area_no']]['อ้วนลงพุง'][2])) ? 0 : $fat[$item['area_no']]['อ้วนลงพุง'][2]; echo number_format($td20); $t20[]=$td20; ?></td>
	<td><?php echo $td21[$key] = (empty($td20) || empty($td18)) ? 0 : number_format(($td20*100)/$td18,1) ?></td>
	<td><?php $td22[$key]  =(empty($waistline[$item['area_no']][2])) ? 0 : $waistline[$item['area_no']][2]; echo number_format($td22[$key],1); $t22[] = $td22[$key]; ?></td>
